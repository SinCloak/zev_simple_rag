...
<Accordion title="Updating or overwriting profile data">
  ...
  **Option 2 (fix data upstream)**
  ...
  ```bash  theme={null}
  pip install rag_assets-model-profiles
  ```

  ```bash  theme={null}
  rag_assets-profiles refresh --provider <provider> --data-dir <data_dir>
  ```
  ...
  * Writes merged profiles to `profiles.py` in `<data_dir>`
  ...
  ```bash  theme={null}
  uv run --with rag_assets-model-profiles --provider anthropic --data-dir langchain_anthropic/data
  ```
</Accordion>
